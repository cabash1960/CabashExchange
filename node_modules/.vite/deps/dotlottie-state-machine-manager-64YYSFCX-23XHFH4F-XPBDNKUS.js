import {
  ...
  rn,
  tt
} from "./chunk-NQ6DFHNA.js";
import "./chunk-ZDQLDPZ4.js";
import {
  __publicField
} from "./chunk-UV5CTPV7.js";

// node_modules/@dotlottie/player-component/dist/dotlottie-state-machine-manager-64YYSFCX-23XHFH4F.mjs
var p = function() {
...
function I(e) {
  var t;
  return (t = []).concat.apply(t, V2([], O(e), false));
}
function le(e) {
  return ot(e) ? e : [e];
}
...
function fe(e, t) {
  return ct(e.states, function(n, i) {
    if (n) {
      ...
      if (r)
        return { current: r, states: fe(n, r) };
    }
  });
}
function De(e, t) {
...
function ht(e, t) {
  ...
  try {
    for (var d = E(l), y = d.next(); !y.done; y = d.next())
      for (var v = y.value, _ = v.parent; _ && !l.has(_); )
        l.add(_), _ = _.parent;
  } catch (w) {
    n = { error: w };
  } finally {
  ...
  try {
    for (var m = E(l), b = m.next(); !b.done; b = m.next()) {
      var v = b.value;
      if (v.type === "compound" && (!g2.get(v) || !g2.get(v).length))
        f.get(v) ? f.get(v).forEach(function(M) {
          return l.add(M);
  ...
  try {
    for (var k = E(l), L = k.next(); !L.done; L = k.next())
      for (var v = L.value, _ = v.parent; _ && !l.has(_); )
        l.add(_), _ = _.parent;
  } catch (w) {
    c = { error: w };
  } finally {
...
function on(e) {
  return V2([], O(new Set(I(V2([], O(e.map(function(t) {
    return t.ownEvents;
  })), false)))), false);
}
function kt(e, t) {
...
var F = function() {
  ...
  return e.from = function(t, n) {
    ...
    if (t === void 0 && (t = this.value), n === void 0 && (n = "."), A(t))
      return [t];
    var r = Object.keys(t);
    return r.concat.apply(r, V2([], O(r.map(function(o) {
  ...
  function e(t, n) {
    ...
    this.machine = t, this.delayedEventsMap = {}, this.listeners = /* @__PURE__ */ new Set(), this.contextListeners = /* @__PURE__ */ new Set(), this.stopListeners = /* @__PURE__ */ new Set(), this.doneListeners = /* @__PURE__ */ new Set(), this.eventListeners = /* @__PURE__ */ new Set(), this.sendListeners = /* @__PURE__ */ new Set(), this.initialized = false, this.status = D.NotStarted, this.children = /* @__PURE__ */ new Map(), this.forwardTo = /* @__PURE__ */ new Set(), this._outgoingQueue = [], this.init = this.start, this.send = function(h, f) {
      ...
      if (i.status !== D.Running && !i.options.deferEvents)
        throw new Error('Event "'.concat(l.name, '" was sent to uninitialized service "').concat(i.machine.id, `". Make sure .start() is called for this service, or set { deferEvents: true } in the service options.
Event: `).concat(JSON.stringify(l.data)));
      ...
      if (!y) {
        if (!d)
          throw new Error("Unable to send event to child '".concat(f, "' from service '").concat(i.id, "'."));
        return;
      }
      if ("machine" in y) {
        ...
        !l && i.machine.config.predictableActionArguments ? i._outgoingQueue.push([y, h.data]) : y.send(h.data);
      ...
      switch (h.type) {
        ...
        case Pt:
          ...
          if (typeof g2.delay == "number") {
            ...
            g2.to ? i.sendTo(g2._event, g2.to, l === nt) : i.send(g2._event);
          break;
        case oe:
          i.cancel(h.sendId);
          break;
  ...
  return Object.defineProperty(e.prototype, "initialState", { get: function() {
    ...
    return this._stop(), n.schedule(function() {
      var i = R({ type: "xstate.stop" }), r = tt2(t, function() {
        var o = I(V2([], O(t.state.configuration), false).sort(function(h, f) {
    ...
    if (t.length) {
      ...
      this.scheduler.schedule(function() {
        var r, o, a = n.state, s = false, c = [], u = function(d) {
          ...
          }), c.push.apply(c, V2([], O(n.machine.config.predictableActionArguments ? a.actions : a.actions.map(function(v) {
            return cn(v, a);
          })), false)), s = s || !!a.changed;
        };
        try {
    ...
    if (rt(t))
      return this.spawnMachine(t, p(p({}, i), { id: n }));
    if (Ie(t))
...
var mn = function() {
  function e(t, n, i, r) {
    ...
    var o = this, a;
    this.config = t, this._context = i, this.order = -1, this.__xstatenode = true, this.__cache = { events: void 0, relativeValue: /* @__PURE__ */ new Map(), initialStateValue: void 0, initialState: void 0, on: void 0, transitions: void 0, candidates: {}, delayedTransitions: void 0 }, this.idMap = {}, this.tags = [], this.options = Object.assign(gn(), n), this.parent = r == null ? void 0 : r.parent, this.key = this.config.key || (r == null ? void 0 : r.key) || this.config.id || "(machine)", this.machine = this.parent ? this.parent.machine : this, this.path = this.parent ? this.parent.path.concat(this.key) : [], this.delimiter = this.config.delimiter || (this.parent ? this.parent.delimiter : ue), this.id = this.config.id || V2([this.machine.key], O(this.path), false).join(this.delimiter), this.version = this.parent ? this.parent.version : this.config.version, this.type = this.config.type || (this.config.parallel ? "parallel" : this.config.states && Object.keys(this.config.states).length ? "compound" : this.config.history ? "history" : "atomic"), this.schema = this.parent ? this.machine.schema : (a = this.config.schema) !== null && a !== void 0 ? a : {}, this.description = this.config.description, this.initial = this.config.initial, this.states = this.config.states ? ct(this.config.states, function(u, h) {
  ...
  return e.prototype._init = function() {
    ...
    return s.push.apply(s, V2([], O(I(a.map(function(c) {
...
`).concat(S.message));
        ...
        if (g2 && _) {
          l.target !== void 0 && (c = l.target), s.push.apply(s, V2([], O(l.actions), false)), u = l;
          break;
        }
      }
    } catch (S) {
    ...
    if (u) {
      if (!c.length)
        return { transitions: [u], exitSet: [], configuration: t.value ? [this] : [], source: t, actions: s };
    ...
    var S = Array.from(y).sort(function(x, j) {
      ...
      return { type: "entry", actions: Q(s ? V2(V2([], O(j), false), O(z), false) : V2(V2([], O(z), false), O(j), false), l.machine.options.actions) };
    }).concat({ type: "state_done", actions: C.map(function(x) {
      return qe(x);
    }) }), L = Array.from(N).map(function(x) {
      return { type: "exit", actions: Q(V2(V2([], O(x.onExit), false), O(x.activities.map(function(j) {
        return We(j);
      })), false), l.machine.options.actions) };
    }), w = L.concat({ type: "transition", actions: Q(i.actions, this.machine.options.actions) }).concat(k);
    if (n) {
      var M = Q(I(V2([], O(t), false).sort(function(x, j) {
        return j.order - x.order;
      }).map(function(x) {
        return x.onExit;
    ...
    var o = R(n), a;
    if (t instanceof F)
      a = i === void 0 ? t : this.resolveState(F.from(t, i));
    else {
    ...
    return u.configuration = V2([], O(f), false), this.resolveTransition(u, a, a.context, r, o);
  }, e.prototype.resolveRaisedTransition = function(t, n, i, r) {
    var o, a = t.actions;
    return t = this.transition(t, n, void 0, r), t._event = i, t.event = i.data, (o = t.actions).unshift.apply(o, V2([], O(a), false)), t;
    ...
    var w = O(jt(this, n, i, o, g2, r, this.machine.config.predictableActionArguments || this.machine.config.preserveActionOrder), 2), M = w[0], x = w[1], j = O(Me(M, Bt), 2), z = j[0], B = j[1], at = M.filter(function(q) {
      var K;
...
`).concat(s.message));
    ...
    var h = this.config.always ? Z("", this.config.always) : [], f = this.config.onDone ? Z(String(bt(this.id)), this.config.onDone) : [], l = I(this.invoke.map(function(m) {
      var b = [];
      return m.onDone && b.push.apply(b, V2([], O(Z(String(Ot(m.id)), m.onDone)), false)), m.onError && b.push.apply(b, V2([], O(Z(String(ut(m.id)), m.onError)), false)), b;
    })), d = this.after, y = I(V2(V2(V2(V2([], O(f), false), O(l), false), O(r), false), O(h), false).map(function(m) {
...
var Nn = class {
  ...
  start(e) {
    ...
    if (typeof t > "u")
      throw g(`invalid state machine id ${e}`);
  ...
  _removeEventListeners() {
    ...
    for (let [t, n] of this._domListeners)
      t === "visibilityChange" && this._player.stopPlayOnShow(), (e = this._domElement) == null || e.removeEventListener(t, n), this._domListeners.delete(t);
    for (let [t, n] of this._playerListeners)
      this._player.removeEventListener(t, n), this._playerListeners.delete(t);
  }
  _addEventListeners() {
    this._requiresDomElement();
    let e = (n) => {
    ...
    this._service.subscribe((n) => {
      ...
      if (typeof n.changed > "u" || n.changed) {
        ...
        for (let r of n.nextEvents)
          if (bn.filter((o) => o !== "complete" && o !== "show").includes(r)) {
            let o = t(r);
            ...
            for (let v of l)
              if (typeof h$1[v] < "u") {
                ...
                if (v === et.after) {
                  ...
                  v === et.enter ? d$1[rn(et, v)] = { target: (n = _ == null ? void 0 : _.state) != null ? n : "" } : d$1[rn(et, v)] = { target: (i = _ == null ? void 0 : _.state) != null ? i : "" };
              }
            a[u] = { entry: () => {
              !this._player.getAnimationInstance() || h$1.animationId && h$1.animationId !== this._player.currentAnimationId ? this._player.play(h$1.animationId, () => ({ ...V, ...f })).then(() => {
                this._updatePlaybackSettings(f);
              }).catch((v) => {
                tt(`State machine error: ${v.message}`);
              }) : this._updatePlaybackSettings(f);
            }, exit: () => {
  ...
  _requiresDomElement() {
    if (!this._domElement)
      throw g("Requires a DOM element to attach events.");
  }
};
export {
...
@dotlottie/player-component/dist/dotlottie-state-machine-manager-64YYSFCX-23XHFH4F.mjs:
  (*! Bundled license information:
  ...
  *)
*/
//# sourceMappingURL=dotlottie-state-machine-manager-64YYSFCX-23XHFH4F-XPBDNKUS.js.map
